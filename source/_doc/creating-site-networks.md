...
terminus site hostnames add example-network.com --site=example-network --env=live
```
Bare domains on live require a [redirect to remove `www.`](/docs/redirect-incoming-requests/#redirect-to-a-common-domain) added to `wp-config.php`, and [special DNS configuration](/docs/#serving-sites-from-bare-domains-with-http).
